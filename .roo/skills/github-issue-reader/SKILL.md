...
projeto (`.github/SCOPES.md` e `.github/CONVENTIONS.md`), para:

- Extrair o ID `[SXX-YY]` e o número da issue.
- Definir o tipo do commit (`feat`, `fix`, `test`, etc.).
- Definir o scope correto (um dos 12 em `SCOPES.md`).
- Gerar o nome da branch no padrão aceito pelo CI.
- Preparar o contexto para a skill `gitflow` gerar o commit e o PR.
...
- "S05-01" -> precisa mapear via `.github/SCOPES.md` ou buscar nas issues abertas por titulo contendo `[S05-01]`
...
Apresente o resultado e prepare o terreno para a skill `gitflow`.
...
## Passo 7: Executar ou Sugerir Próximos Passos

Baseado no `06-execution-mode.md`, se o usuário pediu para **trabalhar** na issue, você deve:

1. Criar a branch localmente (ação segura):
   ```bash
   git checkout -b <prefixo>/<scope>/<SXX-YY>-<descricao>
   ```
2. Implementar as alterações necessárias nos arquivos.
3. Usar a skill `gitflow` (comando `/commit`) para gerar a mensagem de commit final.
4. **Perguntar** antes de fazer o commit, push ou abrir o PR.

Se o usuário apenas pediu para **analisar**, pergunte:
...
Nunca execute commit, push ou PR sem confirmação explícita.
...
1. NUNCA criar branch, push, commit ou PR sem confirmacao (exceto criar a branch localmente se o usuário pedir para implementar).
